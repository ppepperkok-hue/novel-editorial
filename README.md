...
面向「番茄小说」副业场景的 AI 文学编辑部：11 位人格化 Agent 协作创作（工作日节拍 + 晨会分派 + 行为选择权 + 记忆/关系/情绪）+ Python 记忆/知识层（SQLite）+ 番茄 HTTP 发布 + 桌面控制台。

目标场景是每天定时自动生成并发布两章，月预算 100 元内无人值守。Cookie 失效、预算超限、重复触发、章节数不足等异常会自动熔断并写入告警，不会把错误内容发上平台。

当前状态：**端到端链路已真实跑通**（新书会议 → 建书 → 日更生成 → 番茄发布 → 阅读数据回流 → 全员日记 → 知识库同步）；2026-08-11 完成去 n8n 迁移与人格化大工程（R0–R4），业务由本地 Python 调度器执行，n8n 工作流 JSON 归档于 `docs/legacy/` 作为回退备份。
...
- **行为选择权**：主编分派默认生效并注入写手任务；写手可接受/拒绝/提替代方案（`TASK_RESPONSE_MODE` 灰度）；留言可携带 rework/clarify/defer 决策并自动产生行动项；认领任务交稿即兑现、未兑现涨摩擦。
- **编辑部工作日**：开工即上班（晨会 → 主产出 → 待决策 → 手动收工），首页可选今日主题（写稿 N 章/整理日/开会日/自由安排），收工自动写日记、留待办、发公告；三类桌面提醒（未开工/未发稿/完成待决策）。
- **Agent 自主性**：安全动作白名单（写报告/更新知识草稿/投议题/认领任务/提方案），越权动作拒绝并留痕；周会观点变化沉淀为「观点演化」时间线。
...
- **九类会议**：例会/剧情碰头会/选题会/单章会诊/数据复盘会/收尾会/危机处理会/知识分享会/茶水间闲聊，材料、议程、会后动作按类型路由（事故会产经验草稿、学习会产知识草案、收尾会记录建议），写作指令注入下一章上下文。
...
| 页面 | 内容 |
| --- | --- |
| 仪表盘 | 编辑部状态（待命/工作中/待决策/已收工/有遗留）、今日任务、上次执行、最近会议、流程卡片（开工/周会/知识管家）、预算进度、健康检查、完读率、最近发布、热点选题、开工主题选择 |
