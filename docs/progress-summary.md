...
- 远端：本地 HEAD `113246d`；`origin/main` 为 `0a86458`（N9 收口归档提交待网络恢复后补推），工作区干净
...
- 无 AI 味：AI 味逐句定位与改写建议（N8）、语料校准阈值建议与 `--apply` 幂等写回（N9）。
...
| 单元 | 优先级 | 依赖 | 说明 |
| --- | --- | --- | --- |
| N11 示例编辑部与剧本 | P2 | M3 已满足 | 开箱示例班子与作品（下一候选） |
| N12 图形面板三扇窗 | P2 | N10 已满足 | 用户明确后置 |
| N15 灵感素材库 | P2 | N5 已满足 | 灵感、素材、意象随手存 |
| N19 一致性自动核查 | P1 | N5 已满足 | 审稿自动对照设定库检查矛盾 |
| N20 文风参考学习 | P1 | F14 已满足 | 从参考语料提取风格特征 |
