...
失败策略（显式留痕优先）：所有写作/审阅/提炼 LLM 节点失败走 error 分支 → 「失败留痕」
节点 → 汇总运行结果，为未落库的那一轨补写 `draft` + 错误记录（publish_logs 可见），
不再有无痕丢章；A 轨写手等失败时 B 轨因依赖 A 轨承接而不发布，但失败显式落库；
质量门失败双轨各自落库；守护/读者/主编缺失时降级放行并带 note 标记（机械检查 +
逻辑审稿通过仍发布，属防中断取舍）；Planner/生成作品资料失败直接短路（Executions 可见）。
...
- **无活跃书时日更报错**：算章节号匹配不到当前书（未建书/绑定失效）会显式失败告警，
  不再静默取番茄账号第一本书。
- **双轨失败兜底依赖 n8n 分支执行顺序**：合并兜底从「整理剧情A」触发、读取质量门
  A/B 的 passed——成立前提是 n8n 深度优先执行到兜底时质量门已运行；代码层面已按
  此模型校验，真机首次日更时需实际验证一次双轨失败场景。
- **失败留痕机制**：写手/润色/审稿/读者/主编/提炼/整理/质量门均配置
