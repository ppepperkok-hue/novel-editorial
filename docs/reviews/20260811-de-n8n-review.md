...
本次审查覆盖去 n8n 迁移的全部改动：`tools/editorial_daily.py`（调度器）、`tools/editorial_steps.py`（节点逻辑复刻）、`tools/flow_graph.py` / `tools/export_flow_html.py`（链路可视化）、`tools/daily_runs.py`（本地留痕与 legacy 同步）、`novel_editorial/services/control.py`（控制层去 n8n）、`novel_editorial/web_api.py`（executions/flow/export 端点）、`scripts/install_daily_task.ps1` / `install_autostart.ps1` / `watch_daily.py`（触发与自启）、前端 `DashboardPage/ExecutionsPage/FlowPage/Shell/App`。
...
1. `scripts/watch_daily.py` 原第 12 行硬编码 `DB = r"E:\code\novel-editorial\demo.db"`，换机即失效。**已修复**：改为 `ROOT / "demo.db"`，同时去掉对 n8n execution_entity 与 `N8N_API_KEY` 的依赖，改读 `daily_runs`。

### P3

2. `novel_editorial/services/control.py` 的 `apply_schedule` 原用 `config.DB_PATH.name` 传给计划任务脚本，数据库若位于子目录会丢失路径层级。**已修复**：改用 `os.path.relpath(config.DB_PATH, ROOT)`。
...
- 退役验证：n8n 主进程与 task-runner 已停止（`N8N_OFFLINE`）；启动文件夹仅剩 `NovelEditorial-api.vbs`；`docs/legacy/novel_workflow.json`（121,745 B）与 `architect_weekly.json` 已归档。
