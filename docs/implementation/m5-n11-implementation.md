...
1. **不配 key 可跑**：默认配置（`NOVEL_EMBEDDING_BACKEND=local`，即项目默认）下示例生成不调用任何 LLM/embedding 接口（mock 语义），预置内容确定性写入，重复执行结果可复现；用户显式配置 `api` embedding 后端时，设定/记忆写入会按既有检索保鲜语义调用 embedding 接口（有 key 真实调用、无 key 降级告警）。
...
- 一条核心函数把「活的编辑部」造出来，任何已有查询命令都能看到内容；示例质量门使用固定内部阈值 `EXAMPLE_QUALITY_THRESHOLD`，保证任何用户阈值配置下默认草稿都是 draft 待拍板（显式参数可覆盖）；CLI 不接。
...
- 收口（2026-08-20）：R1（87caff9 + ee5ffdd + cb1c1b3）、R2（740b178 + 17ed31d）、R3（README + usage）全部完成并独立审查收敛；全量 892 测试、smoke_m3、stress_m3 全绿；审查链归档 docs/reviews/20260820-M5N11R1-initial.md / 20260820-M5N11R1-fix.md / 20260820-M5N11R1-fix2.md / 20260820-M5N11R2-initial.md / 20260820-M5N11R2-fix.md / 20260820-M5N11R3.md。N11 正式收口。
