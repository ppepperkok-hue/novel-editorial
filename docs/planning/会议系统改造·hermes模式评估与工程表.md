...
| 2 | free 模式：自主发言调度器（事件驱动 + 接话决策器 + 并行发言池 + 沉默权 + 冷场/主动议题 + 主席调停），与 rounds 并存 | 无人 @ 也能自然接龙；允许沉默；冷场 30s 内有动作；「总结一下」产出报告落盘；rounds 不回归 | 接话决策单测；并行/间隔/沉默权测试；冷场计时测试；锁与并发专项；步间审查 | mode 字段默认 rounds |
...
- 会议支持 `rounds`（现状）与 `free`（自主发言群聊）两种模式，一键切换且互不破坏。
- free 模式下无人 @ 也能自然接龙：编辑按关注点自主接话、有权沉默、可主动发起议题；主席负责拉回跑题、冷场推进、按需总结。
- 老板可在直播中 @指定编辑（必答）、插入指示、响应审批；所有操作实时可见（SSE）。
