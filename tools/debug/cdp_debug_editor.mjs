...
ws.onmessage = (ev) => {
  ...
  if (msg.id && pending.has(msg.id)) {
    ...
    if (url.includes("/rest/") && status >= 400) {
      events.push(`HTTP ${status} ${url}`);
    } else if (url.includes("/rest/workflows")) {
      events.push(`HTTP ${status} ${url}`);
    }
...
const evalResult = await send("Runtime.evaluate", {
  expression: `JSON.stringify({href: location.href, title: document.title, bodyText: document.body.innerText.slice(0, 3000)})`,
  returnByValue: true,
});
