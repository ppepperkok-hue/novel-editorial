...
**人格化大工程 R0–R4（血肉层）**：
- R0 打通说话→行为：主编分派生效、认领兑现、审稿 notes 落库可见、议题转行动项；
- R1 行为选择权：任务接受/拒绝/替代方案（灰度）、消息决策回路（rework/clarify/defer）、认领兑现结算；
- R2 社会性：关系快照进分派、信任优先、打回措辞随摩擦、心情注入语气；
- R3 自主与成长：agency 安全白名单、观点演化时间线、被打回率趋势；
- R4 编辑部生活：工作日状态机与决策点、九类会议泛化、仪式感（开工/收工/里程碑公告）、三类桌面提醒。

测试基线：437 后端 + 16 前端全绿；审查报告 `docs/reviews/20260811-persona-blood-review.md`。

**自由度补完（R5，2026-08-12）**：即时重写回环（消息驱动、REWORK_MAX 防循环、超限回落行动项）、记忆按角色过滤注入、memory_used 使用痕迹落库可观测。测试基线：448 后端 + 16 前端全绿。
