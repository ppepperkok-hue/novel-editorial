...
- **瓶颈 1 CLI 单文件**：`src/novel_editorial/cli/app.py` 846 行、12 个命令组；后续扩展维护成本上升，建议按命令组拆模块（涉及目录结构，属地基变更，先评估方案再实施）。
- **瓶颈 2 检索全表扫描**：50 万字量级下 memory search / inspect 约 1.4–1.6s，当前可用但随数据线性增长；量级再涨需索引 / FTS / 向量检索。
- **瓶颈 3 测试耗时**：全量 pytest 60.4s（覆盖率模式 68s），随功能线性增长；CI 可分片、本地可子集。
